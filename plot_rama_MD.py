...
forcefields = []
forcefields.append(sys.argv[1])
#forcefields = ['03w', '99sb', 'c36', 'c22', 'Absinth', 'high_T']
for i in forcefields:
	aafrom = 23
	aato = 45
	if i == 'Absinth' or i == 'high_T':
		aafrom = 2
		aato =  24
	phis = []
	psis = []
	...
	infilename = ""
	if i != 'high_T':
		infilename = "PDBs_"+i+"/"+i+"_all_conformations.pdb"
		#infilename = "PDBs_"+i+"/conformation1.pdb"
	else:
		infilename = "PDB_"+i+"/"+i+"_all_conformations.pdb"

	structure = Bio.PDB.PDBParser(QUIET=True).get_structure(i, infilename)
	print i, "parsed"
...
#for i in range(0,99):
	...
	plt.xlabel('residues')
	plt.xlim(23,45)
	plt.legend( (p1[0], p2[0], p3[0]), ('Alpha', 'L-alpha', 'Beta-str.', 'PPII'),
			bbox_to_anchor=(0.115, 0.99, 0.8, 0.002), loc=8, ncol=4 )
	plt.savefig(i+"_sec_struct_every.png", dpi=600)
	plt.clf()

	print i, "figures plotted"
